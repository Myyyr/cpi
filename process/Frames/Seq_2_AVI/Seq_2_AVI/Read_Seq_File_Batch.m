...
input_path_parent = '/media/loic/Storage/work/cpi/data/caltech/test/seq';
outPath_parent = '/media/loic/Storage/work/cpi/data/caltech/test/videos';

input_folders = dir(input_path_parent);
disp(input_folders);
disp(length(input_folders));
for folder_cnt = 1:length(input_folders)
    ...
    i = 2;
    if(input_folders(folder_cnt).name == 'set06')
        i = 14;
    end
    disp(i);
    ...
    for cnt = [1]
        ...
        output_image_fold = fullfile('/media/loic/Storage/work/cpi/data/caltech/test/frames/',input_folders(folder_cnt).name)
        output_image_file = [output_image_fold,'/', image_files(cnt).name(1:end-4)]
